...
export const tradeService = {
  ...
  async executeTrade(symbol: string, side: 'BUY' | 'SELL', quantity: number, testMode = true): Promise<any> {
    ...
    const apiKey = safeProcess.env.BINANCE_API_KEY || localDb.getConfig().api_key_encrypted;
    const apiSecret = safeProcess.env.BINANCE_API_SECRET || localDb.getConfig().api_secret_encrypted;
